...
class EnhancedWebCrawler:
    ...
    def looks_like_affiliate_url(self, url):
        ...
        affiliate_params = [
            'aff', 'affid', 'affiliateid', 'ref', 'refid', 'referral', 
    ...
    # The next part will continue with check_url_for_keywords and ALL other methods.
    ...
    # The full set of crawling, element checking, result adding, extraction, redirect, keyword, and deep search methods would continue here.
    # Please reply "continue" for the next block, which will include:
    # - check_element
    # - add_result
    # - extract_categories
    # - get_main_pages
    ...
    # --- START OF DEEP SEARCH LOGIC FOR THE 15 LINKS ---

    def deep_search_shopbuttler(self, url):
        try:
            parsed = urlparse(url)
            if "shopbuttler.com" in parsed.netloc or "travelnotes.org" in parsed.netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                candidate_links = []
                for a in soup.find_all('a', href=True):
                    href = a['href']
                    if "shop" in href or "shops" in href or "q=" in href or "country=" in href:
                        full_url = urljoin(url, href)
                        candidate_links.append(full_url)
                for shop_page in candidate_links:
                    try:
                        shop_resp = self.session.get(shop_page, timeout=15)
                        shop_soup = self.get_soup(shop_resp.text)
                        for link in shop_soup.find_all('a', href=True):
                            href = link['href']
                            if "/visit" in href and "site=" in href:
                                full_link = urljoin(shop_page, href)
                                parsed2 = urlparse(full_link)
                                query = parse_qs(parsed2.query)
                                if "site" in query:
                                    site_val = query["site"][0]
                                    if "gowithguide" in site_val.lower():
                                        matched_kws = self.get_matched_keywords(full_link)
                                        if matched_kws:
                                            self.add_result(
                                                source_url=shop_page,
                                                matched_url=full_link,
                                                element='a',
                                                attribute='href',
                                                content=link.get_text() + " " + full_link,
                                                keywords=matched_kws,
                                                location_type='deep_affiliate_link'
                                            )
                    except Exception as e:
                        self.status_messages.append(f"Shopbuttler deep link error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Shopbuttler detection error: {str(e)}")

    def deep_search_eventflow(self, url):
        try:
            if "eventflow.app" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                event_links = []
                for a in soup.find_all('a', href=True):
                    href = a['href']
                    if "/events/" in href:
                        event_links.append(urljoin(url, href))
                for event_url in event_links:
                    try:
                        event_resp = self.session.get(event_url, timeout=15)
                        event_soup = self.get_soup(event_resp.text)
                        for link in event_soup.find_all('a', href=True):
                            href = link['href']
                            if "gowithguide" in href:
                                matched_kws = self.get_matched_keywords(href)
                                if matched_kws:
                                    self.add_result(
                                        ...
                                        element='a',
                                        attribute='href',
                                        content=link.get_text() + " " + href,
                                        keywords=matched_kws,
                                        location_type='deep_affiliate_link'
                                    )
                        for script in event_soup.find_all('script'):
                            if script.string:
                                urls_in_js = re.findall(r'(https?://[^\s\'"]+)', script.string)
                                for js_url in urls_in_js:
                                    if "gowithguide" in js_url:
                                        matched_kws = self.get_matched_keywords(js_url)
                                        if matched_kws:
                                            self.add_result(
                                                source_url=event_url,
                                                matched_url=js_url,
                                                element='script',
                                                attribute='text',
                                                content=script.string[:150],
                                                keywords=matched_kws,
                                                location_type='deep_affiliate_link'
                                            )
                    except Exception as e:
                        self.status_messages.append(f"Eventflow deep event error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Eventflow deep crawl error: {str(e)}")

    def deep_search_price(self, url):
        try:
            if "price.com" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                candidate_links = []
                for a in soup.find_all('a', href=True):
                    href = a['href']
                    if "cashback" in href or "coupon" in href or "store=" in href:
                        candidate_links.append(urljoin(url, href))
                for cand in candidate_links:
                    try:
                        cand_resp = self.session.get(cand, timeout=15)
                        cand_soup = self.get_soup(cand_resp.text)
                        for link in cand_soup.find_all('a', href=True):
                            href = link['href']
                            if "gowithguide" in href:
                                matched_kws = self.get_matched_keywords(href)
                                if matched_kws:
                        ...
                        self.status_messages.append(f"Price.com deep link error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Price.com detection error: {str(e)}")

    def deep_search_tritonbranders(self, url):
        try:
            if "tritonbranders.com" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                search_links = [a['href'] for a in soup.find_all('a', href=True) if "search" in a['href']]
                for search_url in search_links:
                    full_search_url = urljoin(url, search_url)
                    try:
                        resp2 = self.session.get(full_search_url, timeout=15)
                        soup2 = self.get_soup(resp2.text)
                        for a in soup2.find_all('a', href=True):
                            ...
                            if "gowithguide" in href:
                                ...
                                if matched_kws:
                                    self.add_result(
                                        source_url=full_search_url,
                                        matched_url=href,
                                        element='a',
                                        attribute='href',
                                        content=a.get_text() + " " + href,
                                        keywords=matched_kws,
                                        location_type='deep_affiliate_link'
                                    )
                    except Exception as e:
                        self.status_messages.append(f"Tritonbranders search error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Tritonbranders detection error: {str(e)}")

    def deep_search_awin1(self, url):
        try:
            resp = self.session.get(url, timeout=15)
            soup = self.get_soup(resp.text)
            for a in soup.find_all('a', href=True):
                href = a['href']
                if "awin1.com/cread.php" in href and "v=87121" in href:
                    matched_kws = self.get_matched_keywords(href)
                    if matched_kws:
                        self.add_result(
                            source_url=url,
            ...
            self.status_messages.append(f"AWIN1 detection error: {str(e)}")

    def deep_search_storecategory(self, url):
        try:
            if "storecategory.com" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                finder_links = [a['href'] for a in soup.find_all('a', href=True) if "finder" in a['href'] or "business" in a['href']]
                for finder_url in finder_links:
                    full_finder_url = urljoin(url, finder_url)
                    try:
                        resp2 = self.session.get(full_finder_url, timeout=15)
                        soup2 = self.get_soup(resp2.text)
                        for a in soup2.find_all('a', href=True):
                            href = a['href']
                            if "gowithguide" in href:
                        ...
                        self.status_messages.append(f"Storecategory finder error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Storecategory detection error: {str(e)}")

    def deep_search_buybutwhere(self, url):
        try:
            if "buybutwhere.com" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                where_links = [a['href'] for a in soup.find_all('a', href=True) if "where" in a['href']]
                for where_url in where_links:
                    full_where_url = urljoin(url, where_url)
                    try:
                        resp2 = self.session.get(full_where_url, timeout=15)
                        soup2 = self.get_soup(resp2.text)
                        for a in soup2.find_all('a', href=True):
                            href = a['href']
                            if "gowithguide" in href:
                        ...
                        self.status_messages.append(f"Buybutwhere where error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Buybutwhere detection error: {str(e)}")

    def deep_search_searchfor(self, url):
        try:
            if "searchfor.org" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                search_links = [a['href'] for a in soup.find_all('a', href=True) if "search" in a['href']]
                for search_url in search_links:
                    ...
                    try:
                        ...
                        self.status_messages.append(f"Searchfor search error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Searchfor detection error: {str(e)}")

    def deep_search_skimlinks(self, url):
        try:
            if "skimlinks.com" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                merchant_links = [a['href'] for a in soup.find_all('a', href=True) if "merchant" in a['href'] or "gowithguide" in a.get_text()]
                for m_url in merchant_links:
                    full_merchant_url = urljoin(url, m_url)
                    try:
                        resp2 = self.session.get(full_merchant_url, timeout=15)
                        soup2 = self.get_soup(resp2.text)
                        for a in soup2.find_all('a', href=True):
                            href = a['href']
                            if "gowithguide" in href:
                        ...
                        self.status_messages.append(f"Skimlinks merchant error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Skimlinks detection error: {str(e)}")

    def deep_search_knoji(self, url):
        try:
            if "knoji.com" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                sub_links = [a['href'] for a in soup.find_all('a', href=True) if "gowithguide" in a['href'] or "review" in a.get_text().lower()]
                for s_url in sub_links:
                    full_sub_url = urljoin(url, s_url)
                    try:
                        resp2 = self.session.get(full_sub_url, timeout=15)
                        soup2 = self.get_soup(resp2.text)
                        for a in soup2.find_all('a', href=True):
                            href = a['href']
                            if "gowithguide" in href:
                        ...
                        self.status_messages.append(f"Knoji deep link error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"Knoji detection error: {str(e)}")

    def deep_search_onlymelbourne(self, url):
        try:
            if "onlymelbourne.com.au" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                melbourne_links = [a['href'] for a in soup.find_all('a', href=True) if "gowithguide" in a.get_text().lower()]
                for m_url in melbourne_links:
                    full_melb_url = urljoin(url, m_url)
                    try:
                        resp2 = self.session.get(full_melb_url, timeout=15)
                        if resp2.status_code in (301, 302) and 'Location' in resp2.headers:
                            redirect_url = resp2.headers['Location']
                            final_url = self.resolve_redirects(redirect_url)[0]
                        else:
                            final_url = resp2.url
                        if "gowithguide" in final_url:
                            matched_kws = self.get_matched_keywords(final_url)
                            if matched_kws:
                                self.add_result(
                                    source_url=full_melb_url,
                                    matched_url=final_url,
                                    element='a',
                                    attribute='href',
                                    content=full_melb_url + " " + final_url,
                                    keywords=matched_kws,
                                    location_type='deep_affiliate_link'
                                )
                    except Exception as e:
                        self.status_messages.append(f"OnlyMelbourne deep link error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"OnlyMelbourne detection error: {str(e)}")

    def deep_search_timetorv(self, url):
        try:
            if "timetorv.com" in urlparse(url).netloc:
                resp = self.session.get(url, timeout=15)
                soup = self.get_soup(resp.text)
                review_links = [a['href'] for a in soup.find_all('a', href=True) if "review" in a.get_text().lower()]
                for r_url in review_links:
                    full_r_url = urljoin(url, r_url)
                    try:
                        resp2 = self.session.get(full_r_url, timeout=15)
                        soup2 = self.get_soup(resp2.text)
                        for a in soup2.find_all('a', href=True):
                            href = a['href']
                            if "gowithguide" in href:
                        ...
                        self.status_messages.append(f"TimeToRV deep link error: {str(e)}")
        except Exception as e:
            self.status_messages.append(f"TimeToRV detection error: {str(e)}")

    # --- END OF DEEP SEARCH LOGIC FOR THE 15 LINKS ---
    ...
    def process_url(self, url):
        ...
        # Deep search for all 15 links
        self.deep_search_shopbuttler(url)
        self.deep_search_eventflow(url)
        self.deep_search_price(url)
        self.deep_search_tritonbranders(url)
        self.deep_search_awin1(url)
        self.deep_search_storecategory(url)
        self.deep_search_buybutwhere(url)
        self.deep_search_searchfor(url)
        self.deep_search_skimlinks(url)
        self.deep_search_knoji(url)
        self.deep_search_onlymelbourne(url)
        self.deep_search_timetorv(url)
...
def main():
    st.set_page_config(page_title="Enhanced Web Crawler", layout="wide")
    ...
    if st.session_state.running and st.session_state.crawler:
        ...
        if st.session_state.results:
            with results_container.container():
                st.header(f"Matches Found ({len(st.session_state.results)})")
                for i, result in enumerate(st.session_state.results[-5:], 1):
                    st.write(f"**Match {i}:**")
                    st.write(f"Source URL: {html.escape(result['source_url'])}")
                    st.write(f"Matched URL: {html.escape(result['matched_url'])}")
                    st.write(f"Keyword: {html.escape(result['keyword'])} | Location: {html.escape(result['location_type'])}")
                    st.write(f"Element: {html.escape(result['element'])} [{html.escape(result['attribute'])}]")
                    if result['content']:
